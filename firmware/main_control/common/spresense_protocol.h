// Compatible with existing Spresense AQB1 firmware; isolated from tether protocol.
#pragma once
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
public:
  ...
  bool accept(const Nav &n, uint64_t received) {
    ...
    // A 64-byte frame at 115200 bps can arrive in ~5.6 ms. The old 20 ms
    // minimum rejected real packets (~10 ms). Keep a guard, upper bound,
    // three consecutive epochs and PPS interval checks; absolute phase needs bench validation.
    if(!(n.flags&time_valid) || !edge || age<2000 || age>800000 || n.usec>1000) {
