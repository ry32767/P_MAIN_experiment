...
| 判定 | リセット後の自動開始・短時間保存は成功。ユーザーによる電源断・再通電後の自動記録も確認。PC非接続の単独電源運転は未実施。開始前に既存エラーを確認 |
...
RX、水温、水圧、テザー、LCDはこの実験の対象外（従来申告では未接続）。別作業での構成変更はこの試験では追跡していない。[全配線](../../docs/HARDWARE.md)。最初はUSB通電中のリセット試験、その後ユーザーにUSBを含む給電の切断・再接続を依頼し、完了の申告を得た。電源立上り・電圧降下の測定は行っていない。
...
ユーザーが給電の切断・再接続を完了したと回答した後、COM7をDTR/RTSともFalseで開き、リセットや開始コマンドを送らず約5秒間観測した。初回受信時にすでに`sd=1`、保存23,776行で、27,683行まで増加した。qで停止後は27,868行、STOPPED_REMOVE_SDを確認。取得・保存・サブコアの各エラーは0だった。起動直後のファイル名メッセージは観測開始前なので未捕捉。保存ファイル全量検証ではIMU 27,839行（28.799289秒、平均966.62Hz）、GPS 27行で、両方CRC・連番エラー0だった。

[電源投入後の状態](evidence/power_boot.txt)、[GPS保存検証](evidence/gps_power_validation.json)、[IMU保存検証](evidence/imu_power_validation.json)。

## 電源を入れただけで動くか

**今回の接続・給電条件では、開始コマンドなしで電源入れ直し後に自動記録することを確認した。** 電源の切断操作そのものはユーザー申告、こちらでは再接続後のカウンター再初期化と保存動作を観測した。PCを一切接続しない電源だけでの運転は未試験。
...
ファーム変更なしのため今回はビルド試験を追加していない。未実施：USB未接続運転、長時間保存、既存エラーの原因切り分け、カードの実ファイルシステム形式確認。
